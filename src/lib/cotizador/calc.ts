...
import { OBRA_DURACION_MINIMA_DIAS } from "@/lib/cotizador/business-rules"
...
/** Factor de días: aplica a tarifas diarias o mensuales; evento/turno/unidad no se multiplican. */
export function factorDias(unidadCobro: string, dias: number) {
  if (unidadCobro === "dia") return Math.max(1, dias)
  if (unidadCobro === "mes") return Math.max(1, Math.ceil(dias / 30))
  return 1
}

export function validarDuracionObra(
  items: ItemEnPaquete[],
  catalogo: CatalogItem[],
  duracionDias: number | undefined
): { valido: boolean; mensaje?: string } {
  for (const it of items) {
    const item = catalogo.find((c) => c.id === it.catalogItemId)
    if (!item || item.categoria !== "obras") continue
    const tarifa = item.tarifas.find((t) => t.id === it.tarifaId)
    if (!tarifa || tarifa.unidadCobro !== "mes") continue
    const dias = duracionDias ?? 1
    if (dias < OBRA_DURACION_MINIMA_DIAS) {
      return {
        valido: false,
        mensaje: "Los productos de obra requieren una duración mínima de 30 días.",
      }
    }
  }
  return { valido: true }
...
export function nextNumeroSolicitud(
  existentes: { numeroReferencia: string }[],
  year = new Date().getFullYear()
) {
  const prefix = `BOGA-REF-${year}-`
  let max = 0
  for (const e of existentes) {
    if (!e.numeroReferencia.startsWith(prefix)) continue
    const n = Number(e.numeroReferencia.slice(prefix.length))
